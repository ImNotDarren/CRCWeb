...
export default function EditActivity({ router, mid }) {
  ...
  const handlePopup = (cid) => () => {
    const content = module.crcAssignments.find((c) => c.id === cid);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      moduleCopy.crcAssignments.find((c) => c.id === selectedContent.id).assignment = value;
  ...
  const handleCreate = () => {
    ...
    fetch(`${SERVER_URL}/crc/moduleAssignments/create`, {
      ...
      .then(data => {
        const modulesCopy = JSON.parse(JSON.stringify(modules));
        const moduleCopy = modulesCopy.find((m) => m.id === mid);
        moduleCopy.crcAssignments.push(data);
        dispatch({ type: 'UPDATE_MODULES', value: modulesCopy });
        setLoading(false);
        reset();
      })
  ...
  const handleDelete = async () => {
    ...
    fetch(`${SERVER_URL}/crc/moduleAssignments/remove`, {
      ...
      .then(data => {
        const modulesCopy = JSON.parse(JSON.stringify(modules));
        const moduleCopy = modulesCopy.find((m) => m.id === mid);
        moduleCopy.crcAssignments = moduleCopy.crcAssignments.filter((c) => c.id !== selectedContent.id);
        dispatch({ type: 'UPDATE_MODULES', value: modulesCopy });
        reset();
      })
      .catch(error => {
  ...
  return (
    <View style={styles.container}>
      {
        module?.crcAssignments?.map((content, index) => {
          return (
            <CustomizeMenuItem
              key={content.id}
